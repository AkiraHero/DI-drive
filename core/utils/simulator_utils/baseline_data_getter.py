...
import carla
import weakref
from .baseline_data_filter import DEPTH_RGB
from .baseline_data_filter import filter_actors
...
class BaselineDataWrapper:
    def __init__(self, cfg):
        ...
        self.town_name = cfg.town_name
        self.map_root = cfg.map_root
    ...
    def init(self, world, hero_actor):
        self.world = world
        self.hero_actor = hero_actor
        self.spawn_depth_sensor()

    def update_all_actors(self):
        self.all_actors = self.world.get_actors()

    def spawn_depth_sensor(self):
        config = DEPTH_RGB
        ...
        self.depth_sensor = self.world.spawn_actor(sensor_bp, transform, attach_to=self.hero_actor)  # 安装到自车上
        weak_self = weakref.ref(self)
        self.depth_sensor.listen(lambda event: BaselineDataWrapper._on_depth_camera_event(weak_self, event))

    @staticmethod
    def _on_depth_camera_event(weak_self, event):
        self = weak_self()
        if not self:
            return
        self.depth_data = event
    ...
    def get_crop(self, img, ego_pix, meters_behind, meters_ahead, meters_left, meters_right, resolution):
        ...
        return img[slice(int(ego_pix[1] - pix_ahead), int(ego_pix[1] - pix_behind)), slice(int(ego_pix[0] + pix_left),
                                                                                           int(ego_pix[
                                                                                                   0] + pix_right)), ...]
        ...
        for line in kitti_label:
            ...
            for pix_x in range(pix_x_min, pix_x_max):
                for pix_y in range(pix_y_min, pix_y_max):
                    w_dis = self.euclidean_distance(np.tan(theta + math.pi), img_y - np.tan(theta + math.pi) * img_x,
                                                    [pix_x, pix_y])
                    l_dis = self.euclidean_distance(-1 / np.tan(theta + math.pi),
                                                    img_y + 1 / np.tan(theta + math.pi) * img_x, [pix_x, pix_y])
            ...
            if (label_corners < 0).sum() > 0 or (label_corners[:, 0] > 352).sum() == 4 or (
                    label_corners[:, 1] > 352).sum() == 4:
                continue
            ...
            for i in range(0, 3):
                points = []
                if i >= min_dist_index:
                    if (b_list[i + 1] < 0 and 0 <= b_list[min_dist_index] < 400) or (
                            b_list[min_dist_index] < 0 and 0 <= b_list[i + 1] < 400):
                        points = [label_corners[i + 1], label_corners[min_dist_index],
                                  intersection_points[min_dist_index], [0, 0], intersection_points[i + 1]]
                    elif (b_list[i + 1] >= 400 and 0 <= b_list[min_dist_index] < 400) or (
                            b_list[min_dist_index] >= 400 and 0 <= b_list[i + 1] < 400):
                        points = [label_corners[i + 1], label_corners[min_dist_index],
                                  intersection_points[min_dist_index], [0, 400], intersection_points[i + 1]]
                    elif b_list[i + 1] >= 400 and b_list[min_dist_index] < 0:
                        points = [label_corners[i + 1], label_corners[min_dist_index],
                                  intersection_points[min_dist_index], [0, 0], [0, 400], intersection_points[i + 1]]
                    elif b_list[min_dist_index] >= 400 and b_list[i + 1] < 0:
                        points = [label_corners[i + 1], label_corners[min_dist_index],
                                  intersection_points[min_dist_index], [0, 400], [0, 0], intersection_points[i + 1]]
                    else:
                        points = [label_corners[i + 1], label_corners[min_dist_index],
                                  intersection_points[min_dist_index], intersection_points[i + 1]]
                else:
                    if (b_list[i] < 0 and 0 <= b_list[min_dist_index] < 400) or (
                            b_list[min_dist_index] < 0 and 0 <= b_list[i] < 400):
                        points = [label_corners[i], label_corners[min_dist_index], intersection_points[min_dist_index],
                                  (0, 0), intersection_points[i]]
                    elif (b_list[i] >= 400 and 0 <= b_list[min_dist_index] < 400) or (
                            b_list[min_dist_index] >= 400 and 0 <= b_list[i] < 400):
                        points = [label_corners[i], label_corners[min_dist_index], intersection_points[min_dist_index],
                                  [0, 400], intersection_points[i]]
                    elif b_list[i] >= 400 and b_list[min_dist_index] < 0:
                        points = [label_corners[i], label_corners[min_dist_index], intersection_points[min_dist_index],
                                  [0, 0], [0, 400], intersection_points[i]]
                    elif b_list[min_dist_index] >= 400 and b_list[i] < 0:
                        points = [label_corners[i], label_corners[min_dist_index], intersection_points[min_dist_index],
                                  [0, 400], [0, 0], intersection_points[i]]
                    else:
                        points = [label_corners[i], label_corners[min_dist_index], intersection_points[min_dist_index],
    ...
    def get_HDmap(self, ego_pose):
        ...
        for layer in [self.global_map, self.global_waypoint]:
            ...
            layerReshape = cv2.resize(layerRotation, (int(width / self.world_scale * resolution),
                                                      int(height / self.world_scale * resolution)))  # carla map的分辨率不统一，因此需要调整
            layerCrop = self.get_crop(layerReshape, [width / self.world_scale * resolution // 2,
                                                     height / self.world_scale * resolution // 2], meters_behind,
                                      meters_ahead, meters_left, meters_right, resolution)
    ...
    def get_frame_data(self):
        hero_actor = self.hero_actor
        kitti_label = filter_actors(self.all_actors, self.depth_data, hero_actor=hero_actor)
        occupancy, occlusion = self.get_occupancy_and_occlusion(kitti_label)
        ego_tranform = hero_actor.get_transform()
        ego_pose = [ego_tranform.location.x, ego_tranform.location.y, ego_tranform.location.z,
                    ego_tranform.rotation.roll, ego_tranform.rotation.pitch, ego_tranform.rotation.yaw]
        map = self.get_HDmap(ego_pose)
        ...
        # debug: imshow
        # cv2.imshow("occupancy", occupancy.astype(np.uint8) * 255)
        # cv2.imshow("occlusion", occlusion.astype(np.uint8) * 255)
        # cv2.imshow("map0", map[0].astype(np.uint8) * 255)
        # cv2.imshow("map0", map[1].astype(np.uint8) * 255)
        # cv2.waitKey(1)
...
#     kitti_label = open("E:/PJLAB_Experiment/Data/carla/carla_new/label_2/007974.txt").readlines()
#     ego_pose = np.load("E:/PJLAB_Experiment/Data/carla/carla_new/pose/007974.npy")
#
