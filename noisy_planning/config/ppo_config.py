from easydict import EasyDict

ppo_config = dict(
    exp_name='ppo21_bev32_lr1e4_bs128_ns3000_update5_train_ft',
    enable_eval=False,
    only_eval=False,
    env=dict(
        collector_env_num=1,
        evaluator_env_num=0,
        simulator=dict(
            town='Town01',
            spawn_manner="random",  # random, near
            spawn_pos_pool_dat="/mnt/lustre/juxiaoliang.vendor/Project/mdl/town5_outter_loop.waypt",
            spawn_pos_radius=30.0,
            delta_seconds=0.1,
            disable_two_wheels=True,
            verbose=False,
            waypoint_num=32,
            planner=dict(
                type='behavior',
                resolution=1,
            ),
            obs=(
                dict(
                    name='birdview',
                    type='bev',
                    size=[160, 160],
                    pixels_per_meter=5,
                    pixels_ahead_vehicle=100,
                ),
                dict(
                    name='toplidar',
                    type='lidar',
                    channels=64,
                    range=32,
                    points_per_second=640000,
                    rotation_frequency=10,
                    upper_fov=10.0,
                    lower_fov=-45.0,
                    position=[0, 0.0, 1.6],
                    rotation=[0, 0, 0],
                    fixed_pt_num=40000,
                ),
                dict(
                    name='linelidar',
                    type='lidar',
                    channels=1,
                    range=20.0,  # if need modify, should modify the env_wrapper.py together
                    points_per_second=3600,
                    rotation_frequency=10,
                    upper_fov=0.0,
                    lower_fov=0.0,
                    position=[2.2, 0.0, 0.6],
                    rotation=[0, 0, 0],
                    fixed_pt_num=1000,
                ),
            ),
        ),
        enable_detector=True,
        detector="pointpillar",  # pointpillar/pvrcnn/centerpoint/fake-pointpillar/fake-pvrcnn/fake-centerpoint/
        col_is_failure=True,
        stuck_is_failure=True,
        ignore_light=True,
        col_solid_lane_is_failure=True,
        ran_light_is_failure=False,
        off_road_is_failure=False,
        wrong_direction_is_failure=False,
        off_route_is_failure=False,
        off_route_distance=15,
        # reward_func="customized_compute_reward",
        reward_func="racing_reward",
        add_camera_vis_to_obs=False,
        #reward_type=['goal', 'distance', 'speed', 'angle', 'failure', 'lane'],
        success_distance=15.0,
        success_reward=0,
        failure_reward=0,
        replay_path='./ppo_video',
        visualize=None,
        manager=dict(
            collect=dict(
                auto_reset=True,
                shared_memory=False,
                context='spawn',
                max_retry=1,
                retry_type='renew',
                step_timeout=120,
                reset_timeout=120,
                # visualize=dict(
                #             type='camera',
                #             outputs=['video'],
                #         ),
            ),
            eval=dict(
                auto_reset=True,
                shared_memory=False,
                context='spawn',
                max_retry=1,
                retry_type='renew',
                step_timeout=120,
                reset_timeout=120,
                visualize=dict(
                            type='camera',
                            outputs=['video'],
                            enabled_id=[0],
                        ),
            )
        ),
        wrapper=dict(
            collect=dict(suite='race', suite_n_vehicles=100, suite_n_pedestrians=0, ),
            eval=dict(suite='race', suite_n_vehicles=100, suite_n_pedestrians=0, ),
        ),
    ),
    server=[
        # dict(carla_host='localhost', carla_ports=[2000, 2002, 2]),
        dict(carla_host='10.5.8.68', carla_ports=[30617, 30627, 2]),
        dict(carla_host='10.5.8.44', carla_ports=[30637, 30647, 2]),
        dict(carla_host='10.5.8.44', carla_ports=[30697, 30707, 2]),
        dict(carla_host='10.5.8.152', carla_ports=[30707, 30717, 2]),
        dict(carla_host='10.5.8.152', carla_ports=[30807, 30817, 2]),
        dict(carla_host='10.5.8.152', carla_ports=[30837, 30847, 2]),
        dict(carla_host='10.5.8.71', carla_ports=[31007, 31017, 2]),
        dict(carla_host='10.5.8.71', carla_ports=[31037, 31047, 2]),
        dict(carla_host='10.5.8.155', carla_ports=[31087, 31097, 2]),
        dict(carla_host='10.5.8.155', carla_ports=[31107, 31117, 2]),
        dict(carla_host='10.5.8.155', carla_ports=[31117, 31127, 2]),
        dict(carla_host='10.5.8.47', carla_ports=[31157, 31167, 2]),
        dict(carla_host='10.5.8.47', carla_ports=[31217, 31227, 2]),
        dict(carla_host='10.5.8.47', carla_ports=[31267, 31277, 2]),
        dict(carla_host='10.5.8.42', carla_ports=[31287, 31297, 2]),
    ],
    policy=dict(
        cuda=True,
        nstep_return=False,
        on_policy=True,
        model=dict(
            obs_shape=[1, 160, 160],
        ),
        learn=dict(
            epoch_per_collect=5,
            batch_size=128,
            learning_rate=0.0001,
            weight_decay=0.0001,
            value_weight=0.5,
            adv_norm=False,
            entropy_weight=0.01,
            clip_ratio=0.2,
            target_update_freq=100,
            learner=dict(
                hook=dict(
                    log_show_after_iter=1000,
                    # load_ckpt_before_run='/cpfs2/user/juxiaoliang/project/DI-drive/noisy_planning/output_log/ppo-train_fake_laser_c2-2022-04-24-19-15-28/ckpt/iteration_63000.pth.tar',
                    save_ckpt_after_iter=3000,
                ),
            ),
        ),
        collect=dict(
            pre_sample_num=3000,
            n_sample=3000,
            collector=dict(
                collect_print_freq=500,
                deepcopy_obs=True,
                transform_obs=True,
            ),
            discount_factor=0.99,
            gae_lambda=0.95,
        ),
        eval=dict(
            evaluator=dict(
                eval_freq=2000,
                n_episode=500,
                stop_rate=1.0,
                transform_obs=True,
                eval_once=True
            ),
        ),
    ),
)

default_train_config = EasyDict(ppo_config)
