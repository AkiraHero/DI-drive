...
def _spawn_vehicles(client, n_vehicles, tm):
    world = client.get_world()
    map = world.get_map()
    _blueprints = world.get_blueprint_library()
    blueprints = _blueprints.filter('vehicle.*')
    ...
    for response in client.apply_batch_sync(batch, True):
        if response.error:
            print('[SIMULATOR]' + str(response.error))


def _spawn_pedestrians(client, n_pedestrians, tm=None) -> None:
    world = client.get_world()
    ...
    while peds_spawned < n_pedestrians:
        ...
        for i in range(len(results)):
            if results[i].error:
                print('[SIMULATOR] Walker ' + str(results[i].error))
            else:
                peds_spawned += 1
        ...
        walker_controller_bp = _blueprints.find('controller.ai.walker')
        ...
        for result in client.apply_batch_sync(batch, True):
            if result.error:
                print('[SIMULATOR] Walker controller ' + str(result.error))
            else:
...
def clean_all_manual_actors(client):
    world = client.get_world()
    ...
    while not_success and try_destroy_cnt < 2:
        ...
        for response in client.apply_batch_sync(destroy_list, True):
            if response.error:
                cur_loop_fail_flag = True
                print('[SIMULATOR]' + str(response.error))
        if cur_loop_fail_flag:
            print("[SIMULATOR] fail to clean all actors.")
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            self.image = pygame.surfarray.make_surface(lidar_img)
        else:
...
def game_loop(args):
    ...
    try:
        ...
        clean_all_manual_actors(client)
        _spawn_pedestrians(client, 30, tm)
        _spawn_vehicles(client, 50, tm)
