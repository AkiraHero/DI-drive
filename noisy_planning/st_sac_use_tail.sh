...
then
      ...
      python simple_rl_train_with_detection.py --use_new_collector -p sac  -n ${sub_out_dir} 2>&1|tee ${real_time_log}
else
      echo "Your config file is $2"
      python simple_rl_train_with_detection.py --use_new_collector -p sac  -n ${sub_out_dir} -d $2 2>&1|tee ${real_time_log}
fi
